...
class JsAutoValidation extends \Zend_Form_Decorator_Abstract
{
    /**
     * Namespace holding the javascript
     * @var String Namespace
     */
    protected $_javascriptNamespace = 'ZendMax';
    
    /**
     * The javascript file validator path
     * @var String javascript validator path
     */
    protected $_javascriptValidatorPath = '/js/lib/ZendMax/Form/Decorator/JsAutoValidation.js';
     ...
     * @return array
     */
    protected function _getJsElementErrorDecorator($element)
    {
        $decorators = array();
        if (
            ...
            foreach($element->getElements() as $e) {
                if ($decorator = $this->_getJsElementErrorDecorator($e)) {
     ...
     * @return array
     */
    protected function _getJsElementFilters(\Zend_Form_Element $element)
    {
        $filters = array();
     ...
     */
    protected function _getJsElementValidators($element)
    {
        ...
        if (
            ...
            foreach($element->getElements() as $e) {
                $validatorsRules[] = $this->_getJsElementValidators($e);
            }
            return $validatorsRules;
        }
        
        $validators = $this->_getElementValidators($element);
    ...
    protected function _buildJsVars()
    {
        ...
        foreach($formElement->getElements() as $element) {
            if($validators = $this->_getJsElementValidators($element)) {
                $elementsToJs[$element->getName()]['validators'] = $validators;
            }
            if ($decorators = $this->_getJsElementErrorDecorator($element)) {
                $elementsToJs[$element->getName()]['decorators'] = $decorators;
            }
            if ($filters = $this->_getJsElementFilters($element)) {
...
SCRIPT;
    ...
    /**
     * Returns true if the element has validators, false otherwise
     * @param \Zend_Form_Element $element
     * @return boolean
     */
    public function _hasValidators(\Zend_Form_Element $element)
    {
        if ($element instanceof \Zend_Form_Element_File) {
            $adapter = $element->getTransferAdapter();
            $validators = $adapter->getValidators();
        } else {
            $validators = $element->getValidators();
        }
        return (bool) count($validators);
    }
    
    /**
     * Returns the element validators
     * @param \Zend_Form_Element $element
     * @return array with validators
     */
    protected function _getElementValidators(\Zend_Form_Element $element)
    {
        if ($element instanceof \Zend_Form_Element_File) {
            $adapter = $element->getTransferAdapter();
            $validators = $adapter->getValidators();
            
            // check which validator (if any) should break on failure and flag as required
            $reflectionAdapter = new \ReflectionObject($adapter);
            $breaksProp = $reflectionAdapter->getProperty('_break');
            $breaksProp->setAccessible(TRUE);
            $breaks = $breaksProp->getValue( $adapter );
            foreach($validators as $name => $object) {
                $validator = $validators[$name];
                if(array_key_Exists($name, $breaks) && $breaks[$name] == 1) {
                    $validator->zfBreakChainOnFailure = 1;
                }
            }
        } else {
            $validators = $element->getValidators();
    ...
    protected function _buildValidatorParameters(\Zend_Validate_Interface $validator)
    {
        ...
        foreach($messageTemplates as &$messageTemplate) {
            $messageTemplate = $view->translate($messageTemplate);
        }
        $params['messageTemplates'] = $messageTemplates;
        return $params;
    }
...
SCRIPT;
    ...
    /**
     * Returns the js validator path
     * @return string Validator path
     */
    public function getJsValidatorPath()
    {
        return $this->_javascriptValidatorPath;
    }
    
    /**
     * Sets the js validatior path
     * @param string $path javascript validator file location
     * @return void
     */
    public function setJsValidatorPath($path)
    {
        $this->_javascriptValidatorPath = $path;
    }
}
