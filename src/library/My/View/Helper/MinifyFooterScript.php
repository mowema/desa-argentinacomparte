 ...
 *
 */
class My_View_Helper_MinifyFooterScript extends Zend_View_Helper_HeadScript
{
    /**
     *
     ...
     * @var string
     */
    protected $_regKey = 'My_View_Helper_MinifyFooterScript';
     ...
     */
    public function minifyFooterScript ($mode = Zend_View_Helper_HeadScript::FILE, $spec = null, $placement = 'APPEND', 
    ...
    public function toString ($indent = null)
    {
            ...
            if (
                ...
                if (count($scripts) > 0) {
                    ...
                    if (is_null($baseUrl) || $baseUrl == '') {
                        $minScript->attributes['src'] = $this->getMinUrl() .
                         '?f=' . implode(',', $scripts);
                    } else {
                        $minScript->attributes['src'] = $this->getMinUrl() .
                         '?b=' . $baseUrl . '&f=' . implode(',', $scripts);
                    }
                    
                    $scripts = array(); // Empty our scripts array
                    $items[] = $this->itemToString(
                    $minScript, '', '', ''); // add the minified item
                }
                $items[] = $this->itemToString($item, $indent, $escapeStart, 
                $escapeEnd); // add this item
            }
        }
        ...
        if (count($scripts) > 0) {
            ...
            // We will create our minify URL here.
            if (is_null($baseUrl) || $baseUrl == '') {
                $minScript->attributes['src'] = $this->getMinUrl() . '?f=' .
                 implode(',', $scripts);
            } else {
                $minScript->attributes['src'] = $this->getMinUrl() . '?b=' .
                 $baseUrl . '&f=' . implode(',', $scripts);
            }
            if ($this->_debug) {
                $minScript->attributes['src'] .= '&debug';
            }
            $scripts = array(); // Empty our scripts array
